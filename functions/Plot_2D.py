...
MODIFICATION HISTORY:
    ...
    Duseong Jo, 19, APR, 2022: VERSION 1.86
    - Add more options to deal with lon/lat labels
    Duseong Jo, 05, AUG, 2022: VERSION 1.90
    - Save grid area for unstructured grids
'''

### Should be changed for sharing with people
### module import - dsj cbar
### cmap check
...
class Plot_2D(object):
    ...
    INPUTS:
           ...
           suppress_lon_labels: hide lon labels
           suppress_lat_labels: hide lat labels
    ...
    def __init__(self, var, lons=None, lats=None, lon_range=[-180,180], lat_range=[-90,90],
                 ...
                 lon_labels=None, lat_labels=None, suppress_lon_labels=False, suppress_lat_labels=False,
        ...
        # ========================================================================
        # font family - to fix some error that font family is not working
        ...
        if self.model_type == 'SE':
            ...
            self.grid_area = np.copy( ds_scrip.grid_area.values )
        ...
        self.suppress_lon_labels = suppress_lon_labels
        self.suppress_lat_labels = suppress_lat_labels
        ...
        if self.model_type == 'FV':
            ...
            self.grid_area_slice = self.grid_area[ self.ncol_inds ]
                    ...
                    if self.sign_sum in [1,2]:
                        ...
                        if nticks_init > 6:
                            ...
                            if check_loop:
                                ...
                                if (self.cmax_od > 3):
                                    linticks = np.linspace( 0, self.cmax_od, num=int(nticks) )
                                    self.colorticks = np.array( [0] + \
                                                             list(10**(linticks[1:])) )
                                else:
    ...
    def plot(self):
        ...
        if self.model_type == 'FV':
            ...
            if 'norm' in self.kwd_pcolormesh:
        ...
        if self.lonlat_info:
            ...
            if self.suppress_lon_labels:
                self.ax.set_xticklabels( [''] * len(self.lonticklabel) )
            if self.suppress_lat_labels:
                self.ax.set_yticklabels( [''] * len(self.latticklabel) )
